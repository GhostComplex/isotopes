...
Isotopes 已经具备多 Agent、workspace、Discord/Feishu transport、skills、session persistence、cron、REST API、daemon、config hot-reload 等大量基础设施，整体架构已经足够接近 OpenClaw。
...
| REST API | `sessions/cron/status` endpoints | 支持 |
| Daemon | `launchd/systemd start/stop/restart/service` | `openclaw gateway` |
| Config hot-reload | (planned, see #717) | 未实现 |
