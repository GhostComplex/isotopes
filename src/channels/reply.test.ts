...
import { parseReply, REPLY_PROMPT } from "./reply.js";

describe("parseReply", () => {
  it("returns text unchanged when no directives present", () => {
    expect(parseReply("hello world")).toEqual({ stripped: "hello world" });
  });

  it("strips [[reply_to_current]] and resolves to triggerMessageId", () => {
    const r = parseReply("[[reply_to_current]]\nhello", "msg-123");
...
describe("REPLY_PROMPT", () => {
  it("teaches both directive forms", () => {
    expect(REPLY_PROMPT).toContain("[[reply_to_current]]");
    expect(REPLY_PROMPT).toContain("[[reply_to: <message-id>]]");
  });
});
