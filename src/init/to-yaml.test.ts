...
describe("toYaml", () => {
  it("emits a commented-out provider when provider is skipped", () => {
    const yaml = toYaml({ provider: { type: "skip" }, channel: { type: "skip" }, codingAgent: "skip" });
  ...
  it("emits group allowlist with no entries (deferred — no allowlist entries emitted)", () => {
    const yaml = toYaml({
      provider: { type: "skip" },
  ...
  it("adds a coding agent (spawnable, claude runner) when claude is enabled", () => {
    const yaml = toYaml({ provider: { type: "skip" }, channel: { type: "skip" }, codingAgent: "claude" });
